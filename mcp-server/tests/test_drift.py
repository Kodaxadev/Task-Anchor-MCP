...
# ---------------------------------------------------------------------------
# Tone-based message formatting (replaces legacy format_* functions)
# ---------------------------------------------------------------------------

class TestToneMessages:

    def test_drift_detected_strict_contains_binary_choice(self):
        from task_anchor.tone import msg, set_tone
        set_tone("strict")
        result = msg("drift_detected",
            score=4, cap=10, snippet="let's switch to redis",
            overlap=0, context=CONTEXT,
        )
        assert "[1]" in result
        assert "[2]" in result

    def test_drift_clear_strict_shows_score(self):
        from task_anchor.tone import msg, set_tone
        set_tone("strict")
        result = msg("drift_clear", score=2, cap=10)
        assert "2" in result
        assert "✓" in result
