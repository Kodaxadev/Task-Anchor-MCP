...
# Strict tone for handler tests — set in each test class that needs it
@pytest.fixture(autouse=True)
def strict_tone(isolated_state):
    """Set strict tone so handler test assertions (written for strict output) pass."""
...
class TestSession:
    ...
    async def test_resume_no_session_returns_guidance(self, isolated_state):
        from task_anchor.handlers_session import handle_session_resume
...
class TestCompletionValidation:
    ...
    @pytest.mark.asyncio
    async def test_strong_semantic_evidence_accepted(self, isolated_state):
        """Stemmed evidence that closely matches exit condition should pass."""
        from task_anchor.handlers import handle_task_lock_create, handle_task_complete
        from task_anchor.config import TASK_LOCK_FILE
        await handle_task_lock_create(lock_args())
        result = await handle_task_complete({
        ...
        assert "COMPLETE" in result[0].text
        assert not TASK_LOCK_FILE.exists()
