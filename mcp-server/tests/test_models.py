...
class TestScopeValidation:
    ...
    def test_similar_prefix_does_not_match(self, tmp_path):
        # /project should NOT match /project_old/file.py
        scope = tmp_path / "project"
        scope.mkdir(parents=True)
        sibling = tmp_path / "project_old" / "file.py"
        lock = make_lock(scope_files=[str(scope)])
        assert lock.validate_scope(str(sibling)) is False
