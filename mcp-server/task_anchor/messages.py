...
# Integrity check — catches missing tone variants at import time (and in tests).
_REQUIRED_TONES = frozenset({"strict", "supportive", "minimal"})
_missing = [
    f"  {key!r}: missing {_REQUIRED_TONES - set(variants)}"
    for key, variants in MESSAGES.items()
    if not _REQUIRED_TONES.issubset(variants)
]
if _missing:
    raise AssertionError(
        "Message registry incomplete — add the missing tone variants:\n"
        + "\n".join(_missing)
    )
