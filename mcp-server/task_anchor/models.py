...
class TaskLock:
    ...
    def validate_scope(self, file_path: str) -> bool:
        ...
        for scope in self.scope_files:
            # Strip trailing separators so the scope_prefix check is consistent.
            abs_scope = os.path.abspath(scope).lower().rstrip("/\\")
            # Two valid matches: file IS the scope path, or file is UNDER it.
            # The os.sep append prevents `/project` from matching `/project_old/`.
            if abs_file == abs_scope or abs_file.startswith(abs_scope + os.sep):
                return True
        return False
